...
sub _parse_bsml {
	my ($self, $bsml) = @_;
	my @files = ($bsml);
	@files = @{$bsml} if( ref( $bsml ) eq 'ARRAY' );

	my $parser = new XML::LibXML;
	foreach my $file ( @files ) {
		my $doc = $parser->parse_file( $file );
		my @fgs = $doc->findnodes("//Feature-group");
		foreach my $fg ( @fgs ) {
			my @fgms = $fg->findnodes("Feature-group-member");
			$self->_add_feature_group_members_to_lookup( @fgms );
		}
	}
...
sub _add_feature_group_members_to_lookup {
	...
	my $tmp;
	foreach my $fgm ( @fgms ) {
		my $feat_id = $fgm->getAttribute( 'featref' );
		my $class = $fgm->getAttribute( 'feature-type' );
