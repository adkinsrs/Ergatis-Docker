...
B<Description:> Creates a BsmlAlignmentFilter object. 
    ...
    Option hmm_data option is only needed when using the 
    'trusted cutoff' filter.

B<Parameters:> Takes a hash ref.
    'file' => '/path/to/file',
    'hmm_db' => '/path/to/coding_hmm.lib.db',
    'filters' => { key => value }
...
sub special_case_filters {
    ...
    if( exists( $self->{'filters'}->{'trusted_cutoff'} ) ) {
        if( !exists( $self->{'_hmm_lookup'} ) || !defined( $self->{'_hmm_lookup'} ) ) {
            die("Option hmm_data should be passed to constructor when using the trusted_cutoff filter");
        } else {
...
sub _init {
    ...
    if( $args->{'hmm_data'} ) {
        ...
        my %tmp;
        tie(%tmp, 'MLDBM', $args->{'hmm_data'}, O_RDONLY) or
            croak("Could not tie hash to $args->{'hmm_data'}");
